...
ARG TERRAGRUNT_VERSION=0.12.24.11
ARG GOTEMPLATE_VERSION=1.02.0
...
RUN wget https://releases.hashicorp.com/terraform/${TERRAFORM_VERSION}/terraform_${TERRAFORM_VERSION}_linux_amd64.zip -O terraform.zip && \
    unzip terraform.zip && mv terraform ${EXE_FOLDER} && rm terraform.zip
RUN wget https://github.com/coveo/terraform-provider-quantum/releases/download/v${TERRAFORM_QUANTUM}/terraform-provider-quantum_${TERRAFORM_QUANTUM}_linux_64-bits.zip -O quantum.zip && \
...
RUN wget https://github.com/coveo/gotemplate/releases/download/v${GOTEMPLATE_VERSION}/_${TERRAGRUNT_VERSION}_linux_64-bits.zip -O gotemplate.zip && \
...
RUN wget https://github.com/kvz/json2hcl/releases/download/v${JSON2HCL_VERSION}/json2hcl_v${JSON2HCL_VERSION}_linux_amd64 -O ${EXE_FOLDER}/json2hcl && \
    chmod +x ${EXE_FOLDER}/json2hcl
