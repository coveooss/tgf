...
If using Rancher Desktop on mac, rancher uses a different default location for its docker socker, so you have to set `export DOCKER_HOST="unix://$HOME/.rd/docker.sock"` in your `~.zshrc`.
